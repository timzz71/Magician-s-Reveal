...
# ============ PATTERNS ============
# Cheat features are grouped into human-readable categories rather than one
# giant flat list. Matching is case-insensitive and word-boundary aware (a
# hit on "Reach" requires "Reach" as its own token, not just those letters
# appearing inside an unrelated word), which is what keeps this from firing
# on ordinary mod code. A couple of overly-generic entries from the original
# lists ("orchard", "Replace Mod", bare "Donut", bare "Refill") were dropped
# on purpose -- they're common English words / short generic tokens that
# showed up in normal mods and added noise, not real signal.

$cheatCategories = [ordered]@{
    "KillAura" = @(
        ...
        "BowAim", "BowAimbot", "BowSpam", "AutoBow",
        "AutoCrit", "CritBypass", "AlwaysCrit", "CriticalHit", "Criticals"
    )
    "Aim Assist" = @(
        "AimAssist", "aim assist", "aim_assist",
        "TriggerBot", "trigger bot", "trigger_bot",
        "AimBot", "AutoAim", "SilentAim", "AimLock", "HeadSnap",
        "SilentRotations", "Silent Rotations"
    )
    "Reach" = @("ReachHack", "ExtendReach", "LongReach", "LagReach", "ReachDisplay")
    "Hitboxes" = @("HitboxExpand")
    "Anti-Knockback" = @("AntiKB", "NoKnockback", "GrimVelocity", "GrimDisabler", "VelocitySpoof", "KBReduce", "Antiknockback")
    "Auto-Crystal" = @(
        "AutoCrystal", "auto crystal", "cw crystal",
        "dontPlaceCrystal", "dontBreakCrystal", "AutoHitCrystal", "autoCrystalPlaceClock",
        "holdCrystal", "stopOnKill", "activateOnRightClick", "placeInterval", "breakInterval"
    )
    "Auto-Anchor" = @(
        "AutoAnchor", "auto anchor", "DoubleAnchor", "HasAnchor", "anchortweaks",
        "anchor macro", "safe anchor", "SafeAnchor", "AirAnchor", "AnchorAction",
        "safe_anchor", "double_anchor", "Totem Hit"
    )
    "Auto-Totem" = @(
        "AutoTotem", "auto totem", "InventoryTotem", "HoverTotem", "hover totem",
        "legittotem", "OffhandTotem", "TotemSwitch"
    )
    "Auto-Pot / Auto-Armor" = @(
        "AutoPot", "auto pot", "AutoArmor", "auto armor", "AutoPotRefill",
        "AutoNethPot", "auto_pot_refill", "auto_neth_pot", "key_pearl", "KeyPearl"
    )
    "Shield Disabler" = @("ShieldDisabler", "ShieldBreaker", "AutoDoubleHand", "auto double hand")
    "Auto-Clicker" = @("AutoClicker", "DoubleClicker", "JitterClick", "ButterflyClick", "CPSBoost")
    "Mace / Axe Swap" = @(
        "AutoMace", "MaceSwap", "SpearSwap", "StunSlam", "AxeSpam", "axe spam",
        "mace_swap", "stun_slam", "quick_strike", "macro_198", "Macro198",
        "JumpReset", "damagetick", "fakePunch", "Fake Punch"
    )
    "Fly" = @("FlyHack", "CreativeFlight", "BoatFly", "PacketFly", "AirJump")
    "Speed" = @(
        "SpeedHack", "BHop", "BunnyHop", "SpeedTimer", "GameSpeed", "SprintReset",
        "StepHack", "FastClimb", "AutoStep", "HighStep", "ElytraSpeed", "InstantElytra", "ElytraSwap"
    )
    "No Fall Damage" = @("AntiFall", "NoFallDamage", "SafeFall")
    "Scaffold / Fast Bridge" = @("ScaffoldWalk", "FastBridge", "BuildHelper", "AutoBridge")
    "Nuker / Fast Break" = @("Nuker", "NukerLegit", "InstantBreak", "GhostHand", "NoSwing")
    "Fast Place" = @("FastPlace", "AirPlace", "AutoPlace", "InstantPlace", "PlaceAssist", "AutoBreach")
    "X-Ray" = @("XRayHack", "OreFinder", "CaveFinder", "OreESP", "NewChunks", "ChunkBorders", "TunnelFinder")
    "ESP" = @("PlayerESP", "MobESP", "ItemESP", "StorageESP", "ChestESP", "Tracers", "NameTagsHack", "TargetHUD", "BlockESP")
    "Web Utility" = @("AutoWeb", "AntiWeb", "web macro", "webmacro", "NoWeb", "lvstrng")
    "Inventory Manipulation" = @("ChestStealer", "InvManager", "InvMovebypass", "FakeInv", "invsee", "ItemExploit", "LootYeeter", "ChestSteal")
    "No-Slow / Water Walk" = @("WaterWalk", "LiquidWalk", "LavaWalk", "NoSlow", "NoSlowdown", "NoSoulSand", "WallHack")
    "Anti-Cheat Bypass" = @(
        "PopSwitch", "FakeLatency", "FakePing", "SpoofRotation", "PositionSpoof",
        ...
        "PacketCancel", "PacketDupe", "PacketSpam", "SelfDestruct", "self destruct", "HideClient",
        "AntiMissClick", "PingSpoof", "ping spoof", "FakeLag", "AntiBot", "NoJumpDelay", "PackSpoof"
    )
    "Auth / License Bypass" = @("AuthBypass", "obfuscatedAuth", "LicenseCheckMixin")
    "Base / Trap Finder" = @("BaseFinder", "Burrow", "SelfTrap", "HoleFiller", "AntiSurround", "AntiBurrow", "AutoCity")
    "TPA / Pearl / Gap" = @("AutoTPA", "WTap", "Wtap", "TargetStrafe", "AutoGap", "AutoPearl", "AutoDtap", "auto_dtap")
    "Fake Identity" = @("Fakenick", "FakeItem", "FreezePlayer")
    "Misc Automation" = @("FastXP", "FastExp", "AutoEat", "AutoMine", "AutoWeapon", "AutoSword", "AutoSprint", "AntiAFK", "AutoRespawn", "AutoFirework")
    "Account / Data Theft" = @(
    ...
    "Cheat Framework Internals" = @(
        "org.chainlibs.module.impl.modules", "org/chainlibs/module/impl/modules",
        "imgui.gl3", "imgui.glfw", "imgui.binding",
        "ClientPlayerInteractionManagerAccessor", "ClientPlayerEntityMixim",
        "dev.gambleclient", "phantom-refmap.json", "xyz.greaj", "xyz/greaj", "Hellion",
        "client-refmap.json", "cheat-refmap.json", "LWFH Crystal",
        "WalksyCrystalOptimizerMod", "WalksyOptimizer", "WalskyOptimizer", "walksy_optimizer",
        "net/minecraft/injection"
    )
    "Known Cheat Client" = @(
        "meteorclient", "meteor-client", "meteordevelopment", "liquidbounce",
        "wurstclient", "sigmaclient", "novoware", "novoclient", "vapeclient",
        "vape.gg", "VapeLite", "impactclient", "aristois", "konas", "rusherhack", "inertia",
        "exhibition", "futureClient", "astolfo", "moonClient", "pandaware",
        "skilled", "azura", "doomsdayclient", "doomsday.jar", "novaclient",
        "IntentClient", "intent.store", "riseclient.com", "rise.today",
        "fdp-client", "VirginClient", "virgin client", "catlean", "CatleanClient",
        ...
        "GrimClient", "grim client", "dqrkis.xyz", "Dqrkis Client", "dqrkis",
        "dev.krypton", "skid.krypton", "jnativehook", "JNativeHook", "GlobalScreen",
        "NativeKeyListener", "api.novaclient.lol", "com/alan/clients", "club/maxstats",
        "wtf/moonlight", "me/zeroeightsix/kami", "net/ccbluex", "net.ccbluex", "today/opai",
        "com/cheatbreaker", "com/moonsworth", "cc/novoline", "dev.virel"
    )
}

# Flat list of every token, derived from the categories above -- used by the
# simpler checks (process names, etc.) that just need "is this a cheat word"
# rather than which specific category it belongs to.
$allCheatTokens = [System.Collections.Generic.List[string]]::new()
foreach ($cat in $cheatCategories.Keys) {
    foreach ($tok in $cheatCategories[$cat]) { [void]$allCheatTokens.Add($tok) }
}
